...
func (l *batchCreator) getUnbatchedTokenFees(ctx context.Context) ([]*peggytypes.BatchFees, error) {
	...
	fn := func() (err error) {
		fees, err = l.bfhdex.UnbatchedTokensWithFees(ctx)
		return
	}
...
func (l *batchCreator) requestTokenBatch(ctx context.Context, fee *peggytypes.BatchFees) {
	...
	l.Log().WithFields(log.Fields{"token_denom": tokenDenom, "token_addr": tokenAddress.String()}).Infoln("requesting token batch on Bfhdex")

	_ = l.bfhdex.SendRequestBatch(ctx, tokenDenom)
}
