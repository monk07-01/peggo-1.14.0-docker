...
import (
	...
	// peggyevents3 "github.com/monk07-01/peggo1.14/solidity/wrappers/TestERC20.sol"
...
type oracle struct {
	*Orchestrator
	lastResyncWithBfhdex time.Time
...
func (l *oracle) observeEthEvents(ctx context.Context) error {
	...
	vs, err := l.bfhdex.CurrentValset(ctx)
	if err != nil {
		l.logger.WithError(err).Warningln("failed to get active validator set on Bfhdex")
		return err
	}
	...
	if !bonded {
		l.Log().WithFields(log.Fields{"latest_bfh_block": vs.Height}).Warningln("validator not in active set, cannot make claims...")
		return nil
	}
	...
	l.Log().WithFields(log.Fields{"claims": len(newEvents), "eth_block_start": l.lastObservedEthHeight, "eth_block_end": latestHeight}).Infoln("sent new event claims to Bfhdex")
	l.lastObservedEthHeight = latestHeight

	if time.Since(l.lastResyncWithBfhdex) >= resyncInterval {
...
func (l *oracle) getEthEvents(ctx context.Context, startBlock, endBlock uint64) ([]event, error) {
	...
	scanEthEventsFn := func() error {
		...
		depositEvents, err := l.ethereum.GetSendToBfhdexEvents(startBlock, endBlock)
		if err != nil {
			return errors.Wrap(err, "failed to get SendToBfhdex events")
		}
...
func (l *oracle) getLastClaimEvent(ctx context.Context) (*peggytypes.LastClaimEvent, error) {
	...
	fn := func() (err error) {
		claim, err = l.bfhdex.LastClaimEventByAddr(ctx, l.cfg.CosmosAddr)
		return
	}
...
func (l *oracle) sendNewEventClaims(ctx context.Context, events []event) error {
	sendEventsFn := func() error {
		...
		lastClaim, err := l.bfhdex.LastClaimEventByAddr(ctx, l.cfg.CosmosAddr)
		if err != nil {
			return err
		}
		...
		for _, event := range newEvents {
			...
			// Considering block time ~1s on Bfhdex chain, adding Sleep to make sure new event is sent
...
func (l *oracle) autoResync(ctx context.Context) error {
	var height uint64
	fn := func() (err error) {
		height, err = l.getLastClaimBlockHeight(ctx, l.bfhdex)
		return
	}
	...
	l.Log().WithFields(log.Fields{"last_resync": l.lastResyncWithBfhdex.String(), "last_claimed_eth_height": height}).Infoln("auto resyncing with last claimed event on Bfhdex")

	l.lastObservedEthHeight = height
	l.lastResyncWithBfhdex = time.Now()
...
func (l *oracle) sendEthEventClaim(ctx context.Context, ev event) error {
	switch e := ev.(type) {
	case *oldDeposit:
		ev := peggyevents2.WrappersSendToCosmosEvent(*e)
		return l.bfhdex.SendOldDepositClaim(ctx, &ev)
	case *deposit:
		ev := peggyevents.WrappersSendToBfhdexEvent(*e)
		return l.bfhdex.SendDepositClaim(ctx, &ev)
	case *valsetUpdate:
		ev := peggyevents2.WrappersValsetUpdatedEvent(*e)
		return l.bfhdex.SendValsetClaim(ctx, &ev)
	case *withdrawal:
		ev := peggyevents2.WrappersTransactionBatchExecutedEvent(*e)
		return l.bfhdex.SendWithdrawalClaim(ctx, &ev)
	case *erc20Deployment:
		ev := peggyevents.WrappersERC20DeployedEvent(*e)
		return l.bfhdex.SendERC20DeployedClaim(ctx, &ev)
	default:
...
type (
	oldDeposit      peggyevents2.WrappersSendToCosmosEvent
	deposit         peggyevents2.WrappersSendToBfhdexEvent
	valsetUpdate    peggyevents2.WrappersValsetUpdatedEvent
	withdrawal      peggyevents2.WrappersTransactionBatchExecutedEvent
	erc20Deployment peggyevents.WrappersERC20DeployedEvent
