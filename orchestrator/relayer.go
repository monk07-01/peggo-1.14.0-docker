...
import (
	...
	"github.com/monk07-01/peggo1.14/orchestrator/ethereum/util"
...
func (l *relayer) relayValset(ctx context.Context, latestEthValset *peggytypes.Valset) error {
	...
	latestBfhdexValsets, err := l.bfhdex.LatestValsets(ctx)
	if err != nil {
		return errors.Wrap(err, "failed to get latest validator set from Bfhdex")
	}
	...
	for _, set := range latestBfhdexValsets {
		sigs, err := l.bfhdex.AllValsetConfirms(ctx, set.Nonce)
		if err != nil {
...
func (l *relayer) shouldRelayValset(ctx context.Context, vs *peggytypes.Valset) bool {
	...
	if vs.Nonce <= latestEthereumValsetNonce.Uint64() {
		l.Log().WithFields(log.Fields{"eth_nonce": latestEthereumValsetNonce, "bfh_nonce": vs.Nonce}).Debugln("validator set already updated on Ethereum")
		return false
	}

	// Check custom time delay offset
	block, err := l.bfhdex.GetBlock(ctx, int64(vs.Height))
	if err != nil {
		l.Log().WithError(err).Warningln("unable to get latest block from Bfhdex")
		return false
	}
	...
	l.Log().WithFields(log.Fields{"bfh_nonce": vs.Nonce, "eth_nonce": latestEthereumValsetNonce.Uint64()}).Debugln("new valset update")
...
func (l *relayer) relayTokenBatch(ctx context.Context, latestEthValset *peggytypes.Valset) error {
	...
	batches, err := l.bfhdex.LatestTransactionBatches(ctx)
	if err != nil {
		return err
	}
	...
	for _, batch := range batches {
		...
		sigs, err := l.bfhdex.TransactionBatchSignatures(ctx, batch.BatchNonce, gethcommon.HexToAddress(batch.TokenContract))
		if err != nil {
			return err
		}
...
func (l *relayer) shouldRelayBatch(ctx context.Context, batch *peggytypes.OutgoingTxBatch) bool {
	...
	if batch.BatchNonce <= latestEthBatch.Uint64() {
		l.Log().WithFields(log.Fields{"eth_nonce": latestEthBatch.Uint64(), "bfh_nonce": batch.BatchNonce}).Debugln("batch already updated on Ethereum")
		return false
	}

	// Check custom time delay offset
	blockTime, err := l.bfhdex.GetBlock(ctx, int64(batch.Block))
	if err != nil {
		l.Log().WithError(err).Warningln("unable to get latest block from Bfhdex")
		return false
	}
	...
	l.Log().WithFields(log.Fields{"bfh_nonce": batch.BatchNonce, "eth_nonce": latestEthBatch.Uint64()}).Debugln("new batch update")
...
func (l *relayer) findLatestValsetOnEth(ctx context.Context) (*peggytypes.Valset, error) {
	...
	cosmosValset, err := l.bfhdex.ValsetAt(ctx, latestEthereumValsetNonce.Uint64())
	if err != nil {
		return nil, errors.Wrap(err, "failed to get Bfhdex valset")
	}
...
type PeggyValsetUpdatedEvents []*peggyevents.WrappersValsetUpdatedEvent
