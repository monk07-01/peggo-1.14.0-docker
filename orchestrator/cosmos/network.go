...
import (
	...
	"github.com/monk07-01/peggo1.14/orchestrator/cosmos/peggy"
	"github.com/monk07-01/peggo1.14/orchestrator/cosmos/tendermint"
...
func (cfg NetworkConfig) loadClientConfig() clientcommon.Network {
	if custom := cfg.CosmosGRPC != "" && cfg.TendermintRPC != ""; custom {
		log.WithFields(log.Fields{"cosmos_grpc": cfg.CosmosGRPC, "tendermint_rpc": cfg.TendermintRPC}).Debugln("using custom endpoints for Bfhdex")
		return customEndpoints(cfg)
	}

	c := loadBalancedEndpoints(cfg)
	log.WithFields(log.Fields{"cosmos_grpc": c.ChainGrpcEndpoint, "tendermint_rpc": c.TmEndpoint}).Debugln("using load balanced endpoints for Bfhdex")
