...
func Test_BatchCreator(t *testing.T) {
	...
	bfhTokenAddress := gethcommon.HexToAddress("0xe28b3B32B6c345A34Ff64674606124Dd5Aceca30")
	...
	testTable := []struct {
...

		{
			...
			orch: &Orchestrator{
				...
				cfg: Config{
					MinBatchFeeUSD:       51.0,
					ERC20ContractMapping: map[gethcommon.Address]string{bfhTokenAddress: "bfhdex"},
				},
...
func Test_Oracle(t *testing.T) {
	...
	testTable := []struct {
...

		{
			...
			orch: &Orchestrator{
				...
				ethereum: MockEthereumNetwork{
					...
					GetSendToCosmosEventsFn: func(_, _ uint64) ([]*peggyevents2.PeggySendToCosmosEvent, error) {
						return nil, errors.New("oops")
					},
				},
			},
...

		{
			...
			orch: &Orchestrator{
				...
				ethereum: MockEthereumNetwork{
					...
					GetSendTobfhdexEventsFn: func(_, _ uint64) ([]*peggyevents.PeggySendTobfhdexEvent, error) {
						return nil, nil
					},
...

		{
			...
			expected:                nil,
			lastObservedEthHeight:   100,
			lastResyncWithbfhdex: time.Now(), // skip auto resync
			orch: &Orchestrator{
				logger:      DummyLog,
				cfg:         Config{EthereumAddr: ethAddr2},
				maxAttempts: maxLoopRetries,
	...
	for _, tt := range testTable {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			o := oracle{
				...
				lastResyncWithbfhdex: tt.lastResyncWithbfhdex,
...
func Test_Relayer_Valsets(t *testing.T) {
	...
	testTable := []struct {
		...
		{
			name:     "failed to get bfhdex block",
			expected: nil,
			orch: &Orchestrator{
				logger:      DummyLog,
				maxAttempts: maxLoopRetries,
